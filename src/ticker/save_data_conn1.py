...
# path = os.path.abspath(os.path.join(output_path,"all_tokens_curr.csv"))
...
# instrument_list = instrument_list[0:int(n/6)]

high_low_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..","..",
                                    config['high_low_files']['conn_1']))
...
count = 0
queue = config['rabbitmq']['queues']['one']
# print(instrument_list)
def on_ticks(ws, ticks):
    # print(len(ticks),(ticks[0]['timestamp']), len(ticks))
    # if(ticks[0]['timestamp'].second == 0):
    # 	print('stock1:' ,(ticks[0]['timestamp']), len(ticks))
    print("conn_1",ticks[0]['exchange_timestamp'],len(ticks))
    if(ticks[0]['exchange_timestamp'].hour < 
    int(config['start_connection']['currency']['hour'])):
        return
    # print(ticks)    
    ...
    if(ticks[0]['exchange_timestamp'].hour == int(config['end_connection']['currency']['hour']) 
    and ticks[0]['exchange_timestamp'].minute == int(config['start_connection']['currency']['minute'])
    and ticks[0]['exchange_timestamp'].second == int(config['start_connection']['currency']['second'])):
